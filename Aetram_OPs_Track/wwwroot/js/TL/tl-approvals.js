...
$(function () {
    ...
    const API = {
        ...
        bulkApprove: '/api/tl/bulk-approve',
        bulkReject: '/api/tl/bulk-reject',
        bulkCorrection: '/api/tl/bulk-correction',
        teams: '/api/tl/teams',
        groups: '/api/tl/groups',
    ...
    async function loadFilters() {
        try {
            const [teamsRes, groupsRes, subGroupsRes, membersRes] = await Promise.all([
                getWithAuth(API.teams),
                getWithAuth(API.groups),
                getWithAuth(API.subGroups),
                getWithAuth(API.teamMembers + '?teamLeadId=' + (currentUser?.userId || 2))
            ]);
            populateTeamFilter(teamsRes.data || teamsRes);
            populateGroupFilter(groupsRes.data || groupsRes);
            populateSubGroupFilter(subGroupsRes.data || subGroupsRes);
            populateMemberFilter(membersRes.data || membersRes);
        } catch (err) {
            populateTeamFilter(DEMO.teams);
            populateGroupFilter(DEMO.groups);
            populateSubGroupFilter(DEMO.subGroups);
            populateMemberFilter(DEMO.teamMembers);
        }
    }
    ...
    async function loadQueue() {
        ...
        try {
            const response = await getWithAuth(API.queue + '?teamLeadId=' + (currentUser?.userId || 2));
            allQueueData = response.data || response;
        } catch (err) {
            allQueueData = DEMO.queue;
        }
        applyFilters();
    }
    ...
    $('#bulk-modal-confirm').on('click', async function () {
        ...
        const endpoints = { approve: API.bulkApprove, reject: API.bulkReject, correction: API.bulkCorrection };

        try {
            await postWithAuth(endpoints[bulkActionType], {
                workLogIds: selectedIds,
                [bulkActionType === 'approve' ? 'approvedBy' : bulkActionType === 'reject' ? 'rejectedBy' : 'requestedBy']: currentUser?.userId || 2,
                comment: comment,
                timestamp: new Date().toISOString()
            });
            showToast(`Bulk ${bulkActionType} successful!`);
        } catch (err) {
            // Demo fallback
            selectedIds.forEach(id => {
                const idx = allQueueData.findIndex(q => q.workLogId === id);
                if (idx >= 0) {
                    allQueueData[idx].approvalStatus = bulkActionType === 'approve' ? 'APPROVED' : bulkActionType === 'reject' ? 'REJECTED' : 'CORRECTION';
                    allQueueData[idx].approvalComment = comment;
                }
            });
            showToast(`Bulk ${bulkActionType} completed!`);
        }
    ...
    async function submitAction(action) {
        ...
        const endpoints = { approve: API.approve, reject: API.reject, correction: API.correction };
        const payloadKeys = { approve: 'approvedBy', reject: 'rejectedBy', correction: 'requestedBy' };

        try {
            await postWithAuth(endpoints[action], {
                workLogId: selectedWorkLogId,
                [payloadKeys[action]]: currentUser?.userId || 2,
                comment: comment,
                timestamp: new Date().toISOString()
            });
            showToast(`EOD ${action === 'approve' ? 'approved' : action === 'reject' ? 'rejected' : 'correction requested'}!`);
        } catch (err) {
            const idx = allQueueData.findIndex(q => q.workLogId === selectedWorkLogId);
            if (idx >= 0) {
                allQueueData[idx].approvalStatus = action === 'approve' ? 'APPROVED' : action === 'reject' ? 'REJECTED' : 'CORRECTION';
                allQueueData[idx].approvalComment = comment;
            }
            showToast(`EOD ${action === 'approve' ? 'approved' : action === 'reject' ? 'rejected' : 'correction requested'}!`);
        }
