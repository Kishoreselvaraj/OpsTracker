...
$(function () {
    ...
    function getValidCategories() {
        if (!categoriesData || !Array.isArray(categoriesData)) {
            console.warn("[CategoryFilter] categoriesData is null/undefined or not an array");
            return [];
        }

        if (!subCategoriesData || !Array.isArray(subCategoriesData)) {
            console.warn("[CategoryFilter] subCategoriesData not available — returning all categories");
            return categoriesData;
        }

        const validCategories = categoriesData.filter((cat) => {
            if (!cat || typeof cat.categoryId === "undefined") return false;
            const hasSubs = subCategoriesData.some(
                (sub) => sub && sub.categoryId == cat.categoryId
            );
            return hasSubs;
        });
    ...
    function buildDraftRowHtml(rowId, displayNumber, log) {
        // FILTERED: Only categories with at least one sub-category
        const validCategories = getValidCategories();
        const catOptions = validCategories
            .map(
    ...
    function loadSubCategoriesForDraftRow(
        ...
        // Defensive: validate jQuery row element
        if (!$sub || $sub.length === 0) {
            console.warn("[CategoryFilter] Sub-category select element not found in row");
            return;
        }

        // Defensive: validate categoryId
        if (!categoryId || categoryId === "" || categoryId === "0") {
            $sub.html('<option value="">Select</option>').prop("disabled", true);
            return;
        }

        // Defensive: validate subCategoriesData
        if (!subCategoriesData || !Array.isArray(subCategoriesData)) {
            console.warn("[CategoryFilter] subCategoriesData not available");
            $sub.html('<option value="">Select</option>').prop("disabled", true);
            return;
        }

        const subs = subCategoriesData.filter((s) => {
            // Defensive: skip null/undefined sub-category entries
            if (!s || typeof s.categoryId === "undefined" || typeof s.subCategoryId === "undefined") {
                return false;
            }
        ...
        console.log("[CategoryFilter] CategoryId:", categoryId);
        console.log("[CategoryFilter] Matched Subs:", subs);
    }
    ...
    async function loadCategories() {
        ...
        if (!categoriesData || !categoriesData.length) categoriesData = getCachedGroups();

        // Log total categories loaded
        const totalCats = categoriesData && Array.isArray(categoriesData) ? categoriesData.length : 0;
        console.log(`[CategoryFilter] Total categories loaded: ${totalCats}`);

        // Log filtered categories info (if subCategoriesData is already loaded)
        if (subCategoriesData && Array.isArray(subCategoriesData)) {
            const validCats = getValidCategories();
            const removedCount = totalCats - validCats.length;
            if (removedCount > 0) {
                const removedCats = categoriesData.filter((cat) => {
                    if (!cat || typeof cat.categoryId === "undefined") return false;
                    return !subCategoriesData.some((sub) => sub && sub.categoryId == cat.categoryId);
                });
                console.warn(`[CategoryFilter] Categories removed (no sub-categories): ${removedCount}`);
                console.warn("[CategoryFilter] Removed category IDs:", removedCats.map((c) => ({ id: c.categoryId, name: c.name })));
            } else {
                console.log("[CategoryFilter] All categories have at least one sub-category");
            }
        }
    }
    ...
    async function loadSubCategories() {
        ...
        if (!subCategoriesData || !subCategoriesData.length) subCategoriesData = getCachedSubGroups();

        // Log total sub-categories loaded
        const totalSubs = subCategoriesData && Array.isArray(subCategoriesData) ? subCategoriesData.length : 0;
        console.log(`[CategoryFilter] Total sub-categories loaded: ${totalSubs}`);

        // If categories already loaded, log filtering info
        if (categoriesData && Array.isArray(categoriesData) && categoriesData.length > 0) {
            const validCats = getValidCategories();
            const removedCount = categoriesData.length - validCats.length;
            if (removedCount > 0) {
                const removedCats = categoriesData.filter((cat) => {
                    if (!cat || typeof cat.categoryId === "undefined") return false;
                    return !subCategoriesData.some((sub) => sub && sub.categoryId == cat.categoryId);
