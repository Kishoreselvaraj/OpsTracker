...
$(function () {
    ...
    async function loadTeamMembers() {
        ...
        try {
            const response = await getWithAuth(API.teamMembers);
            ...
            allMembers = raw.map(m => ({
                ...
                code: '',
                subGroup: '',
                pendingCount: (m.workLogs || m.WorkLogs || []).filter(w => (w.approvalStatus || '').toUpperCase() === 'PENDING').length,
                lastSubmission: '—',
                status: 'active'
            }));
        } catch (err) {
